...
func (s *AuthService) Register(ctx context.Context, email, password, role, code string) (db.CreateUserRow, error) {
	...
	if role != "player" && role != "dm" {
		return db.CreateUserRow{}, fmt.Errorf("role must be 'player' or 'dm', got: '%s'", role)
	}
...
func (s *AuthService) Login(ctx context.Context, email, password string) (string, db.CreateUserRow, error) {
	user, err := s.queries.GetUserByEmail(ctx, email)
	if err != nil {
		return "", db.CreateUserRow{}, fmt.Errorf("invalid email or password")
	}
	...
	if err != nil {
		return "", db.CreateUserRow{}, fmt.Errorf("failed to generate token: %w", err)
	}
